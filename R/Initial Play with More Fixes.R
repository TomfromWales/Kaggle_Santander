...
h2o.gbm(
  ...
  ,y = "target"
  ,x = setdiff(names(train_raw),c("ID","target","rand"))
  ,ntrees = 80
  ,max_depth = 8
  ,learn_rate = 0.1
  ,distribution = "gamma"
...
h2o.saveModel(h2o.getModel("shit_gbm_h2o"),paste0(data_folder,"/models/"),force = TRUE)
...
# Export
write.csv(preds_for_submission,paste0(data_folder,"/submissions/initial_play_submission_set1_with_more_fixes.csv"),row.names = FALSE)
write.csv(preds_for_submission_set2,paste0(data_folder,"/submissions/initial_play_submission_set2_with_more_fixes.csv"),row.names = FALSE)
